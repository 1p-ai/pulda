...
- Notion, Google Drive, NotebookLM, Stitch 2.0, Google Workspace의 역할 분리
- 배포 전 QA 철학
- 클라이언트 인터뷰 처리 방식
...
| knowledge-ops-loop | Notion, Google Drive, NotebookLM, Stitch, Workspace를 이용한 자료 보관, 압축 요약, 기록, 환류 구조 |
...
| Replit AI | 프로젝트 README, AGENTS, dev.md, 작업 지시 템플릿으로 적용 |
| NotebookLM | 프로젝트별 자료 압축 요약, 작업 기준 특화 브리핑 |
| Notion | 프로젝트 사관, 히스토리, 결정 근거, 일일 업데이트 |
| Google Drive | 대용량 원본 자료, 참고 파일, 백업 |
| Stitch 2.0 | 단순 시안 컨셉, HTML 초안 |
| Local Agents | 반복 업무, 낮은 수준의 정리, 초벌 QA, 파일 분류 |
| Hermes Agent | 로컬 자생 에이전트 후보, 반복 처리와 로컬 자동화 |

핵심은 특정 제품 기능명이 아니라, 반복 가능한 작업 지침을 한곳에 정리하고 매번 재사용하는 것이다.

코드 수정 도구를 함께 쓸 때는 `dev.md` 같은 개발 안전 문서를 반드시 포함한다. 특히 기존 코드를 읽지 않고 전체 파일을 재생성하는 방식, 기존 구현을 설명 없이 삭제하는 방식, 라이트/다크 같은 분기 구조를 무시하는 방식은 재발 방지 기준으로 Skill 또는 템플릿에 반영한다.

장기적으로는 로컬 에이전트 운영 기준도 Skill/Template로 분리한다. 단기에는 ChatGPT와 Codex 중심으로 빠르게 처리하고, 맥미니 도입 이후에는 Hermes Agent나 빠른 로컬 모델에게 반복 업무와 낮은 수준의 처리를 맡긴다. 이 전환은 외부 클라우드 의존도를 급격히 끊는 것이 아니라, 생산성을 유지하면서 자생 능력을 늘리는 방향으로 진행한다.
...
- AI가 빠르게 초안을 만들더라도 기초 개념이 부정확하면 Skill 자체가 잘못된 자동화를 강화할 수 있다. 따라서 Skill에는 용어 정의, 판단 근거, 검증 기준을 함께 남긴다.
...
- Google/Notion 기반 자료 보관, 요약, 기록 흐름에서 병목, 중복, 유실 대응 기준이 생겼을 때
- 로컬 에이전트, Hermes Agent, 빠른 로컬 모델을 이용한 반복 업무 이관 기준이 생겼을 때
- AI 노트북, 소버린 AI처럼 미디어/마케팅 용어와 실제 기능 사이의 차이를 설명해야 할 때
- 외부 AI/클라우드 의존 리스크를 낮추기 위한 기술 주권 운영 기준이 생겼을 때
- AI 시대의 기초 역량, 즉 인문학/과학/예체능 기반의 정확한 개념 이해와 사유 경험을 작업 기준에 반영해야 할 때
- QA나 배포 사고를 통해 재발 방지 기준이 생겼을 때
- AI 코드 수정 도구가 기존 구현을 삭제하거나 새 코드로 덮어써 작업 맥락을 잃는 문제가 반복될 때
...
8. 맥미니 같은 로컬 장비 도입 후 local-agent-ops Skill을 추가한다.
